...
test_files = Path(r"V:\New analysis pipeline\231219_152751_wtjx285-2d")
...
def demonstrate_DAQ_issue():

    timestamps = DAQ_data["timestamps"]

    time_between_timepoints = []

    for i, timestamp in enumerate(timestamps):
        if i != 0:
            time_between_timepoints.append(timestamp - timestamps[i-1])

    average_time_between_timepoints = sum(time_between_timepoints) / len(time_between_timepoints)

    print(f"Average time between timepoints is {average_time_between_timepoints} seconds")

    # range of time between timepoints:

    print(f"Range of time between timepoints is {max(time_between_timepoints) - min(time_between_timepoints)} seconds")

    # on a scatter graph, plot the time between timepoints against the timepoint:

    fig, axs = plt.subplots(1, 1)
    axs[0].scatter(timestamps[1:], time_between_timepoints, s = 0.1)
    #plot SPOT2 against timepoint:
    # axs[1].scatter(timestamps, DAQ_data["SPOT2"], s = 0.1)
    plt.show()

def plot_DAQ(DAQ_data, start = 0, end = None):
    ...
    if end == None:
        end = len(DAQ_data["timestamps"])-1
        
    for i, channel in enumerate(channel_order):

        # get channel data and convert to floats:
        data = [float(datapoint) for datapoint in DAQ_data[channel]]
        float_timestamps = [float(timestamp) for timestamp in DAQ_data["timestamps"]]       # that bug i couldn't figure out for ages was because it was plotting strings, not floats.
    ...
    scales_weights = DAQ_data["scales_data"]["weights"]
    scales_timestamps = DAQ_data["scales_data"]["timestamps"]
    threshold = DAQ_data["scales_data"]["mouse_weight_threshold"]
...
print("Plotting...")
plot_all(DAQ_data)
# demonstrate_DAQ_issue()


# print((DAQ_data["timestamps"][-1] - DAQ_data["timestamps"][0])/60)