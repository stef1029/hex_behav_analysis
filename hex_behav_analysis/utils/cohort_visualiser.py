...
# Import the Cohort_folder class (assuming the original script is in the same directory)
# If it's in a different location, update the import accordingly
...
class CohortDateOrganizer(Cohort_folder):
    ...
    def __init__(
        ...
        """
        Initializes the CohortDateOrganizer with the same parameters as Cohort_folder.
        ...
        """
        # Initialize the parent class (Cohort_folder)
        super().__init__(
            cohort_directory, 
            multi=multi,
        ...
        # Additional initializations for this class
        self._date_cache = None  # Cache for get_sessions_by_date results
    
    def get_sessions_by_date(self, refresh_cache=False):
        """
        Organizes all sessions by date, with sessions for each date grouped by mouse.
        ...
        # Dictionary to store sessions organized by date and mouse
    ...
    def print_date_report(self, date=None):
        """
        Prints a formatted report of sessions for a given date or all dates if none specified.
        ...
        # If a specific date is requested, only show that date
        if date is not None:
            if date in sessions_by_date:
                dates_to_show = {date: sessions_by_date[date]}
            else:
                print(f"No sessions found for date {date}")
                return
        else:
            dates_to_show = sessions_by_date
        
        # Print the report
        for date, mice_sessions in dates_to_show.items():
            print(f"\n===== Sessions on {date} =====")
            ...
            for mouse_id, session_ids in mice_sessions.items():
                ...
                for session_id in session_ids:
                    ...
                    # Extract time from session ID (HHMMSS)
                    time_str = session_id[7:13]
                    formatted_time = f"{time_str[:2]}:{time_str[2:4]}:{time_str[4:6]}"
                    
                    print(f"  - {session_id} (Time: {formatted_time}, Phase: {phase})")
...
def main():
    ...
    sessions_by_date = organizer.get_sessions_by_date()
    
    # Print a nicely formatted summary
    print("===== Sessions By Date =====")
    for date, mice_sessions in sessions_by_date.items():
        print(f"\nDate: {date}")
        for mouse, sessions in mice_sessions.items():
            print(f"  Mouse: {mouse}")
            for session in sessions:
                print(f"    - {session}")
    
    # Get and print a summary table
    print("\n===== Date Summary =====")
    summary_df = organizer.get_date_summary()
    print(summary_df)
    
    # Example of getting details for a specific date
    example_date = list(sessions_by_date.keys())[0]  # First date in the data
    print(f"\n===== Detailed information for {example_date} =====")
    date_details = organizer.get_date_session_details(example_date)
    for mouse, sessions in date_details.items():
        print(f"\nMouse: {mouse}")
