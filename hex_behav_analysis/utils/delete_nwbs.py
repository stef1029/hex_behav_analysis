...
def delete_nwb_files(cohort, test_mode=True):
    ...
    for mouse in cohort.cohort["mice"]:
        for session_id in cohort.cohort["mice"][mouse]["sessions"]:
            ...
            print(f"{action_text}: {nwb_path} ({file_size_mb:.2f} MB)")
    ...
    if test_mode:
        ...
        print("To perform actual deletion, set test_mode=False")
...
def main():
    ...
    cohort_directory = r"D:\Behaviour\July_cohort_24\Portable_data"  # Change this to your cohort directory
    
    # Cohort_folder parameters
    multi = True                    # Whether data is split across subfolders (multiple mice)
    portable_data = True            # Use portable_data logic vs. full raw-data logic
    oeab_legacy = True              # Look for legacy OEAB folder structures
    ignore_tests = True             # Skip any session folders that look like test sessions
    use_existing_cohort_info = True # If True and cohort_info.json exists, load from it
    plot = False                    # Whether to produce a cohort summary plot
    
    # Delete NWB files parameters
    test_mode = True                # Set to False to actually delete files
