...
from hex_behav_analysis.Preliminary_analysis_scripts.Arduino_DAQ_import import Arduino_DAQ_Import
...
from hex_behav_analysis.utils.DAQ_to_nwb import *
...
class Process_Raw_Behaviour_Data:
    ...
    def ingest_behaviour_data(self):
        ...
        self.pulses = self.OEAB_data.pulses

        self.DAQ_channel_data = Arduino_DAQ_Import(self.arduino_DAQ_Path, self.pulses).channel_data
        

        self.DAQ_data = self.pulse_ID_sync(self.DAQ_channel_data, self.pulses)
        self.DAQ_data = self.clean_DAQ_data(self.DAQ_data)
        
        self.get_camera_frame_times()

        self.get_scales_data()

        DAQ_plot(self.DAQ_data, self.data_folder_path)
        ...
        self.DAQ_filename = self.arduino_DAQ_Path.parent / f"{self.session_id}_processed_DAQ_data.json"

        with open(self.DAQ_filename, 'w') as f:
            json.dump(self.DAQ_data, f, indent = 4)

        DAQ_to_nwb(DAQ_dict=self.DAQ_data, 
    ...
    def get_camera_frame_times(self):
        ...
        self.camera_pulses = self.OEAB_data.camera_pulses
        ...
        pulse_check = False
        # check if no. pulse_times >= max frame_ID:
        if frame_ID_check:
            if len(self.pulse_times) >= self.frame_IDs[-1]:
                pulse_check = True
            # print(f"pulses: {len(self.pulse_times)}, highest frame: {self.frame_IDs[-1]}")
        ...
        if frame_ID_check:    
            ...
            for frame_ID in self.frame_IDs:
                last_frame_ID = 0
                last_pulse_time = 0
                try:        # if frame_ID is in pulse times, use that pulse time
                    self.frame_times[frame_ID] = self.pulse_times[frame_ID]
                    last_pulse_time = self.frame_times[frame_ID]
                    last_frame_ID = frame_ID
                except KeyError:        # else, simulate pulse times based on framerate to estimate frame times
                    self.frame_times[frame_ID] = ((frame_ID - last_frame_ID)) * (1 / fps) + last_pulse_time
                    simulated_frame_times.append(frame_ID)

        else:    # if frame IDs weren't saved, assume there were no dropped frames and just chop off the extras at the end of the list to avoid index errors
            for i in range(int(self.true_video_framecount)):
                last_frame_ID = 0
                last_pulse_time = 0
                try:
                    self.frame_times[i] = self.pulse_times[i]
                    last_pulse_time = self.frame_times[i]
                    last_frame_ID = i
                except KeyError:
                    self.frame_times[i] = ((i - last_frame_ID)) * (1 / fps) + last_pulse_time
                    simulated_frame_times.append(i)
        ...
        self.frame_times["frame_IDs_saved?"] = frame_ID_check
        self.frame_times["pulse_times_simulated?"] = not pulse_check
        # if frame_IDs are present, but the pulse count wasn't long enough, then make a note of the frames that were simulated
        self.frame_times["simulated_frames"] = f"{simulated_frame_times[0]}:{simulated_frame_times[-1]}" if not pulse_check and frame_ID_check else None
    ...
    def get_scales_data(self):
            ...
            length_timestamps = len(self.DAQ_data["timestamps"])
            ...
            for i in range(0, length_timestamps, length_timestamps // length_scales):
                try:
                    new_scales_timestamps.append(self.DAQ_data["timestamps"][i])
                except IndexError:
                    new_scales_timestamps.append(self.DAQ_data["timestamps"][-1])
            ...
            self.DAQ_data["scales_data"] = self.scales_data
