...
def plot_average_movements(sessions, 
    ...
    def get_data(trials, mouse_id):
        ...
        for trial in trials:
            ...
            if len(df) >= min_frames:
                ...
                df['head_angle'] = df['head_angle'] - (np.pi / 2)
    ...
    # I would now like to take each stack and produce an array of speeds, and accelerations.
    # then I will take an average, find the SEM and plot these, for each bin:
    
    from scipy.signal import savgol_filter
    for key, trajectory_stack in success_data['data'].items():
        if 'speeds' not in success_data:
            success_data['speeds'] = {}
            success_data['accelerations'] = {}
        if key not in success_data['speeds']:
            success_data['speeds'][key] = {'data': [], 'mean': [], 'sem': []}
            success_data['accelerations'][key] = {'data': [], 'mean': [], 'sem': []}
        for trajectory in trajectory_stack:
            # calculate the speed and acceleration of each point in the trajectory
            speeds = np.sqrt(np.diff(trajectory['x'])**2 + np.diff(trajectory['y'])**2)
            window_length = 5
            polyorder = 2
            speeds = savgol_filter(speeds, window_length, polyorder)
            accelerations = np.diff(speeds)
            if success_data['speeds'][key]['data'] == []:
                success_data['speeds'][key]['data'] = speeds
                success_data['accelerations'][key]['data'] = accelerations
            else:
                success_data['speeds'][key]['data'] = np.vstack((success_data['speeds'][key]['data'], speeds))
                success_data['accelerations'][key]['data'] = np.vstack((success_data['accelerations'][key]['data'], accelerations))
        
        # find means and sems:
        success_data['speeds'][key]['mean'] = np.mean(success_data['speeds'][key]['data'], axis=0)
        success_data['accelerations'][key]['mean'] = np.mean(success_data['accelerations'][key]['data'], axis=0)
        success_data['speeds'][key]['sem'] = np.std(success_data['speeds'][key]['data'], axis=0) / np.sqrt(len(success_data['speeds'][key]['data']))
        success_data['accelerations'][key]['sem'] = np.std(success_data['accelerations'][key]['data'], axis=0) / np.sqrt(len(success_data['accelerations'][key]['data']))


    for key, trajectory_stack in unsuccessful_data['data'].items():
        if 'speeds' not in unsuccessful_data:
            unsuccessful_data['speeds'] = {}
            unsuccessful_data['accelerations'] = {}
        if key not in unsuccessful_data['speeds']:
            unsuccessful_data['speeds'][key] = {'data': [], 'mean': [], 'sem': []}
            unsuccessful_data['accelerations'][key] = {'data': [], 'mean': [], 'sem': []}
        for trajectory in trajectory_stack:
            # calculate the speed and acceleration of each point in the trajectory
            speeds = np.sqrt(np.diff(trajectory['x'])**2 + np.diff(trajectory['y'])**2)
            window_length = 5
            ...
            speeds = savgol_filter(speeds, window_length, polyorder)
            accelerations = np.diff(speeds)
            if unsuccessful_data['speeds'][key]['data'] == []:
                unsuccessful_data['speeds'][key]['data'] = speeds
                unsuccessful_data['accelerations'][key]['data'] = accelerations
            else:
                unsuccessful_data['speeds'][key]['data'] = np.vstack((unsuccessful_data['speeds'][key]['data'], speeds))
                unsuccessful_data['accelerations'][key]['data'] = np.vstack((unsuccessful_data['accelerations'][key]['data'], accelerations))
        
        # find means and sems:
        unsuccessful_data['speeds'][key]['mean'] = np.mean(unsuccessful_data['speeds'][key]['data'], axis=0)
        unsuccessful_data['accelerations'][key]['mean'] = np.mean(unsuccessful_data['accelerations'][key]['data'], axis=0)
        unsuccessful_data['speeds'][key]['sem'] = np.std(unsuccessful_data['speeds'][key]['data'], axis=0) / np.sqrt(len(unsuccessful_data['speeds'][key]['data']))
        unsuccessful_data['accelerations'][key]['sem'] = np.std(unsuccessful_data['accelerations'][key]['data'], axis=0) / np.sqrt(len(unsuccessful_data['accelerations'][key]['data']))

    # plot the average speeds:

    # Setup the plot dimensions and subplots
    sorted_angles = sorted(success_data['speeds'].keys())
    ...
    fig, axs = plt.subplots(num_rows, num_columns, figsize=(20, 20), sharex=True, sharey=True)
    fig.suptitle('Average Movement Analysis')

    axs = axs.flatten()  # Flatten in case of multi-row/column layout

    for i, angle in enumerate(sorted_angles):
        ax = axs[i]
        x = scaled_times[angle][:-1]
        
        # Successful Trials
        ax.plot(x, success_data['speeds'][angle]['mean'], color='blue', label='Speed - Success')
        ax.fill_between(x, 
                        success_data['speeds'][angle]['mean'] - success_data['speeds'][angle]['sem'], 
                        success_data['speeds'][angle]['mean'] + success_data['speeds'][angle]['sem'], 
                        color='blue', alpha=0.3)  # Shaded area for SEM

        ax.set_title(f'Angle: {angle}°')
        ax.set_xlabel('Frame')
        ax.set_ylabel('Units/frame')
        ax.legend()
    ...
    for i, angle in enumerate(sorted_angles):
        ax = axs[i]
        x = np.arange(len(unsuccessful_data['accelerations'][angle]['mean']))
        
        ax.plot(x, success_data['accelerations'][angle]['mean'], color='cyan', label='Acceleration - Success')
        ax.fill_between(x, 
                        success_data['accelerations'][angle]['mean'] - success_data['accelerations'][angle]['sem'], 
                        success_data['accelerations'][angle]['mean'] + success_data['accelerations'][angle]['sem'], 
                        color='cyan', alpha=0.3)  # Shaded area for SEM
